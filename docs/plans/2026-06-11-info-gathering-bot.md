...
- [ ] Milestone 4: Discord and Slack webhook posting with `--dry-run`. (completed: `post.py` with Discord embeds + Slack mrkdwn renderers, env-var webhook resolution, 10-item batching, single 429 retry, `--dry-run` preview, `mark_posted` only after a successful post; 20 unit tests pass; full-pipeline dry-run verified live 2026-06-11. Remaining: live Discord acceptance — blocked on the owner creating the three Discord webhooks and putting `DISCORD_WEBHOOK_{AI_PAPERS,AI_NEWS,TECH_NEWS}` in `.env`.)
